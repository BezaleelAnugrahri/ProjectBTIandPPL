...
public class ImageTrackerSample : ARBehaviour
{
	...
	void Update()
	{
		...
		Debug.Log("TrackerTargetLost 404");
		#endregion
		**/
        if (state == null)
        {
		...
		for (int i = 0; i < trackingResult.GetCount(); i++)
		{
            ...
            {
                if (trackable.GetName().Contains(name[j]))
                {
                    PlayerPrefs.SetString("Choose", trackable.GetName());
					Debug.Log("TrackerDetected :" + PlayerPrefs.GetString("Choose"));
                }
            }
            #endregion
			**/	
